...
export const receivingContactSchema = z
  .object({
    full_name: z
      .string()
      .trim()
    ...
    us_state_id: z.string().trim().min(1, "Selecciona el estado."),
    us_city_id: z.string().trim().min(1, "Selecciona la ciudad."),
  ...
  .superRefine((value, context) => {
    ...
    if (value.us_city_id === US_CITY_OTHER_VALUE && !value.city_other) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
  ...
  .transform((value) => ({
    ...value,
    city_other: value.us_city_id === US_CITY_OTHER_VALUE ? value.city_other ?? null : null,
    notes: value.notes ?? null,
    us_city_id: value.us_city_id === US_CITY_OTHER_VALUE ? null : value.us_city_id,
